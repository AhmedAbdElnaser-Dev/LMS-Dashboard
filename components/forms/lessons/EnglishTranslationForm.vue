...
onMounted(async () => {
  if (isEditMode.value && lessonId.value) {
    const enTranslation = lessonsStore.state.lesson?.translations?.find(
      translation => translation.language === 'en',
    )
    ...
    if (enTranslation) {
      form.value.title = enTranslation.title || ''
      form.value.description = enTranslation.description || ''
      form.value.content = enTranslation.content || ''
    }
  }
